...
(function () {
  ...
  function renderLogs(logs) {
    ...
    el.innerHTML = entries.map((e) => {
      const cls = e.subtype === 'hit' ? 'ax-dash-log-success' : e.subtype === 'error' ? 'ax-dash-log-decline' : e.subtype === 'bypass' ? 'ax-dash-log-warning' : 'ax-dash-log-trying';
      let text = '';
      ...
      else if (e.subtype === 'bypass') text = '3DS Bypass: Fingerprint removed from request';
      else if (e.subtype === 'detect') text = '3DS Detected: Secure authentication required';
      else if (e.subtype === '3d_bypass') text = '3D Secure: ' + (e.message || 'Bypass operation');
      else text = e.message || e.subtype || 'Unknown event';
  ...
  document.getElementById('ax-dash-reset-settings')?.addEventListener('click', () => {
    ...
    document.getElementById('ax-dash-bypass-toggle').checked = true;
