...
(function () {
  ...
  function renderLogs(logs) {
    ...
    el.innerHTML = entries.map((e) => {
      const status = e.subtype === 'hit' ? 'success' : e.subtype === 'error' ? 'error' : e.subtype === 'bypass' ? 'warning' : 'info';
      ...
      else if (e.subtype === 'bypass') msg = '3DS fingerprint bypassed';
      else if (e.subtype === 'detect') msg = '3DS request detected';
      else if (e.subtype === '3d_bypass') msg = e.message || '3D Secure bypass';
      else msg = e.message || e.subtype || 'Unknown event';
  ...
  document.getElementById('ax3-refresh-logs')?.addEventListener('click', () => {
    loadData();
    showToast('Logs refreshed');
  });

  document.getElementById('ax3-test-log')?.addEventListener('click', () => {
    testAddLog();
