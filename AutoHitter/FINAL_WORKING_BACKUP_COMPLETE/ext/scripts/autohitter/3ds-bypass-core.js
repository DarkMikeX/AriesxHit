...
let detectCount = 0;
  ...
  window.fetch = async function(input, init) {
    // MASTER BYPASS CONTROL: If bypass is disabled, return original request unchanged
    if (!bypassEnabled) {
    ...
    // Only process Stripe requests if bypass is enabled
    if (isStripe(url)) {
      detectCount++;
  ...
  XHR.prototype.send = function(body) {
    // MASTER BYPASS CONTROL: If bypass is disabled, call original send unchanged
    if (!bypassEnabled) {
  ...
  if (navigator.sendBeacon) {
    ...
    navigator.sendBeacon = function(url, data) {
      // MASTER BYPASS CONTROL: If bypass is disabled, call original sendBeacon unchanged
      if (!bypassEnabled) {
  ...
  function autoClickCaptcha() {
    if (!bypassEnabled) {
      console.log('[HCaptcha] Bypass disabled, skipping click');
      return;
    }
  ...
  function waitForHCaptchaSolved({ intervalMs = 500, timeoutMs = 60000 } = {}) {
      return new Promise((resolve, reject) => {
          ...
          const timer = setInterval(() => {
              // Check if bypass is still enabled
              if (!bypassEnabled) {
                  clearInterval(timer);
                  reject(new Error("Bypass disabled during hCaptcha solving"));
                  console.log('[HCaptcha] ❌ Bypass disabled, stopping hCaptcha monitoring');
                  return;
              }
  ...
  function initHCaptchaAutoClicker() {
    if (!bypassEnabled) {
      console.log('[HCaptcha] Bypass disabled, skipping auto-solver initialization');
      return;
    }
    ...
    const observer = new MutationObserver((mutations) => {
      if (!bypassEnabled) return; // Skip if bypass is disabled
          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (node.matches && node.matches('iframe[src*="hcaptcha"]')) {
                if (!bypassEnabled) return; // Double check bypass is enabled
                console.log('[HCaptcha] hCaptcha iframe detected, starting auto-solve...');
                setTimeout(() => {
                  if (bypassEnabled) { // Final check before clicking
                    autoClickCaptcha();
              ...
              // Also check for checkbox directly
                ...
                if (!bypassEnabled) return; // Check bypass is enabled
                console.log('[HCaptcha] Checkbox element detected, attempting click...');
                setTimeout(() => {
                  if (bypassEnabled) { // Check before clicking
                    autoClickCaptcha();
                    waitForHCaptchaSolved().catch(err => console.log('[HCaptcha] Error:', err.message));
                  }
  ...
  let bypassEnabled = true; // Default enabled

  // Listen for bypass toggle messages from background script
  if (chrome && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.type === 'BYPASS_TOGGLED') {
        const wasEnabled = bypassEnabled;
        bypassEnabled = message.enabled;

        console.log(`%c[3DS BYPASS] ${bypassEnabled ? 'ENABLED' : 'DISABLED'}`, `color: ${bypassEnabled ? '#22c55e' : '#ef4444'}; font-weight: bold`);

        if (!bypassEnabled) {
          console.log('[3DS BYPASS] 3DS bypass and hCaptcha auto-solver are now disabled');

          // Clean up any running intervals
          if (window.__ariesHCaptchaInterval) {
            clearInterval(window.__ariesHCaptchaInterval);
            window.__ariesHCaptchaInterval = null;
          }
        } else if (!wasEnabled && bypassEnabled) {
          console.log('[3DS BYPASS] 3DS bypass and hCaptcha auto-solver are now enabled');
          // Re-initialize hCaptcha auto-clicker if it was disabled
  ...
  if (chrome && chrome.storage && chrome.storage.local) {
    chrome.storage.local.get(['ax_bypass_enabled'], (result) => {
      if (result.ax_bypass_enabled !== undefined) {
        bypassEnabled = result.ax_bypass_enabled;
        console.log(`%c[3DS BYPASS] Initial state: ${bypassEnabled ? 'ENABLED' : 'DISABLED'}`, `color: ${bypassEnabled ? '#22c55e' : '#ef4444'}; font-weight: bold`);
      }
    });
  }
  ...
  // Start hCaptcha auto-clicker only if bypass is enabled
  if (bypassEnabled) {
    initHCaptchaAutoClicker();
  }
