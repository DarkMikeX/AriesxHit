...
(function () {
  ...
  function renderLogs(logs) {
    ...
    if (!el) return;
    // Filter out invalid log entries and only keep valid ones
    const entries = logs.filter((l) => l.type === 'log' && l.subtype && (l.subtype === 'trying' || l.subtype === 'hit' || l.subtype === 'error')).slice(-50).reverse();
  ...
  function updateBypassUI(enabled) {
    const container = document.getElementById('ax-master-switch-container');
    if (container) {
      if (enabled) {
        container.style.borderColor = 'rgba(34, 211, 238, 0.6)';
        container.style.boxShadow = '0 12px 48px rgba(34, 211, 238, 0.3)';
      } else {
        container.style.borderColor = 'rgba(220, 38, 38, 0.6)';
        container.style.boxShadow = '0 12px 48px rgba(220, 38, 38, 0.3)';
      }
    }
  }
