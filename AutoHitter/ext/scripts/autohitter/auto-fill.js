...
(function() {
  ...
  function findCountryField() {
    ...
    for (const el of candidates) {
      ...
      // Skip elements that are clearly name/cardholder fields
      const name = (el.name || '').toLowerCase();
      const id = (el.id || '').toLowerCase();
      const ph = (el.placeholder || '').toLowerCase();
      if (/name|cardholder|card.name|card_name/i.test(name + id + ph)) continue;

      const label = (el.getAttribute('aria-label') || '').toLowerCase();
      const parent = (el.closest?.('[class*="billing"], [class*="address"]')?.textContent || '').toLowerCase();
      if (/country|region|nation/i.test(ph + name + id + label + parent)) return el;
    }
    const allSelects = document.querySelectorAll('select');
    for (const sel of allSelects) {
      const opts = [...sel.options];
      if (opts.some((o) => /mo|macao|macau|china|india|us|united/i.test(String(o.value || o.text)))) return sel;
    }
    return null;
  }
  ...
  window.autoFillCard = function(cardData, bypassMode = false) {
    ...
    // Fill name (use provided name or default)
    const nameField = findField(FIELD_SELECTORS.name);
    if (nameField && !nameField.value) {
      const nameToUse = cardData.name || 'ARIESxHIT';
      results.name = setFieldValue(nameField, nameToUse);
      console.log('[AutoFill] Name:', results.name ? `OK (${nameToUse})` : 'FAILED');
    }
  ...
  // Function to unlock address fields in Stripe checkout
  function unlockAddressFields() {
    console.log('[AutoFill] Attempting to unlock address fields');

    // Try different selectors for unlock buttons/links
    const unlockSelectors = [
      'button[data-testid="address-element-unlock-button"]',
      'button[data-testid="address-unlock-button"]',
      'a[data-testid="address-element-unlock-link"]',
      'button:contains("Enter address manually")',
      'a:contains("Enter address manually")',
      '.address-element-unlock-button',
      '.address-unlock-link',
      '[data-testid*="unlock"]',
    ...
    for (const selector of unlockSelectors) {
      try {
        const unlockBtn = document.querySelector(selector);
        if (unlockBtn && unlockBtn.offsetParent !== null) { // Check if visible
          console.log(`[AutoFill] Found unlock button: ${selector}`);
          unlockBtn.click();
          return true;
        }
      } catch (e) {
    ...
    // Try clicking on address input fields to trigger unlock
    const addressInputs = document.querySelectorAll('input[data-testid*="address"], input[name*="address"], input[placeholder*="address"]');
    for (const input of addressInputs) {
      try {
        if (input.offsetParent !== null) {
          console.log('[AutoFill] Clicking address input to trigger unlock');
          input.click();
          input.focus();
          // Wait a bit and try clicking unlock again
          setTimeout(() => {
            const unlockBtn = document.querySelector('button[data-testid*="unlock"], a[data-testid*="unlock"]');
    ...
    console.log('[AutoFill] No unlock mechanism found - fields may already be unlocked');
    return false;
  }
