...
(function () {
  ...
  function sendHitOnce(card) {
    ...
    console.log('[sendHitOnce] Card breakdown:', {
      cardParam: card,
      pendingCard: state.pendingCard,
      getPendingCard: getPendingCard(),
      lastTriedCard: state.lastTriedCard
    });

    // Ensure we have card data
    const finalCard = card || state.pendingCard || getPendingCard() || state.lastTriedCard;
    console.log('[sendHitOnce] Final card data to send:', finalCard);

    send('CARD_HIT', {
      card: finalCard,
      amount,
      current_url: checkoutUrl,
    });
  }
