...
const CHANNEL_USERNAME = 'ariesxhit'; // Channel username without @

// Join channel keyboard
const JOIN_CHANNEL_KEYBOARD = {
  inline_keyboard: [
    [{ text: '📢 Join Channel', url: 'https://t.me/ariesxhit' }],
    [{ text: '✅ I Joined - Check Membership', callback_data: 'check_membership' }]
  ]
};
...
async function ensureChannelMembership(tgId, firstName, chatId, messageId = null) {
  try {
    console.log(`[CHANNEL_CHECK] Checking membership for user ${tgId} in @${CHANNEL_USERNAME}`);

    const membershipCheck = await checkChannelMembership(BOT_TOKEN, tgId, CHANNEL_USERNAME);

    if (!membershipCheck.ok) {
      console.error('[CHANNEL_CHECK] Failed to check membership:', membershipCheck.error);
      return false;
    }

    if (!membershipCheck.member) {
      console.log(`[CHANNEL_CHECK] User ${tgId} is NOT a member of @${CHANNEL_USERNAME}, showing join message`);

      const joinText = `🚫 <b>ACCESS DENIED</b>\n` +
        `─────────────────\n\n` +
        `Hello <b>${firstName}</b>!\n\n` +
        `You must join our channel to use this bot.\n\n` +
        `📢 <b>@ariesxhit</b>\n\n` +
        `Click the button below to join, then click "✅ I Joined" to continue.`;

      const replyMarkup = { reply_markup: JSON.stringify(JOIN_CHANNEL_KEYBOARD) };

      if (messageId) {
        // Edit existing message
        await editMessageText(BOT_TOKEN, chatId, messageId, joinText, replyMarkup);
      } else {
        // Send new message
        await sendMessage(BOT_TOKEN, chatId, joinText, replyMarkup);
      }

      return false;
    }

    console.log(`[CHANNEL_CHECK] User ${tgId} is a member of @${CHANNEL_USERNAME} ✓`);
    return true;

  } catch (error) {
    console.error('[CHANNEL_CHECK] Error checking membership:', error);
    return false;
  }
}
  ...
  try {
    ...
    if (cb) {
      try {
        ...
        // Check channel membership for all callback queries except 'check_membership'
        if (cb.data !== 'check_membership') {
          const isMember = await ensureChannelMembership(tgId, firstName, chatId, messageId);
          if (!isMember) {
            await answerCallbackQuery(BOT_TOKEN, cb.id, 'Please join the channel first!');
            return;
          }
        }
        ...
        } else if (cb.data === 'check_membership') {
          // Re-check membership after user claims they joined
          const isMember = await ensureChannelMembership(tgId, firstName, chatId, messageId);
          if (isMember) {
            await answerCallbackQuery(BOT_TOKEN, cb.id, 'Welcome! Access granted.');
            // Show main menu after successful membership check
            const result = await editMessageText(BOT_TOKEN, chatId, messageId, getMainMenuText(firstName, tgId), replyMarkup(MAIN_MENU_KEYBOARD));
            if (!result.ok) console.error('Webhook: Failed to show main menu after membership check:', result.error);
          } else {
            await answerCallbackQuery(BOT_TOKEN, cb.id, 'Still not a member. Please join the channel!');
          }
        }
      } catch (error) {
    ...
    if (msg?.text === '/start') {
      try {
        ...
        const isMember = await ensureChannelMembership(tgId, firstName, chatId);
        if (!isMember) {
          console.log(`[START] User ${tgId} not in channel, showing join message`);
          return; // ensureChannelMembership already sent the join message
        }
