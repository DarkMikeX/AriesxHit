...
const LOGIN_TOKEN_TTL_MS = 365 * 24 * 60 * 60 * 1000; // 1 year (effectively until user generates new token)
...
async function checkChannelMembership(botToken, userId, channelUsername) {
  if (!botToken || !userId || !channelUsername) {
    console.error('checkChannelMembership: Missing parameters');
  ...
  try {
    ...
    if (!res.ok) {
      const errorText = await res.text().catch(() => 'Unknown error');
      console.error(`checkChannelMembership: HTTP ${res.status}: ${errorText}`);
      return { ok: false, error: `HTTP ${res.status}: ${errorText}` };
    }
    ...
    if (!data.ok) {
      console.error('checkChannelMembership: Telegram API error:', data.description);
      return { ok: false, member: false, status: null, error: data.description };
    }
    ...
    console.log(`checkChannelMembership: User ${userId} status in @${channelUsername}: ${status}, isMember: ${isMember}`);
    ...
    console.error('checkChannelMembership: Network error:', e.message);
    return { ok: false, error: `Network error: ${e.message}` };
  }
}
